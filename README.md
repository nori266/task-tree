...
**Projects.** Several independent projects can coexist, each a self-contained workspace with its own Tree, Forest, Backlog, and litter. A left sidebar (collapsed by default) switches between them and creates, renames, or deletes them; deletion is undoable via a toast. Each project can wear a **background color** — a swatch on its sidebar row opens a soft palette (or clears the tint), the color rides along in the project index, and the active project's tint washes the whole canvas so you can tell projects apart at a glance. New projects auto-pick a fresh palette color. The active project's stores auto-save under per-project storage keys. Any task can be **moved into another project**, carrying its whole subtree, from a "Move to project" picker in the node panel; the subtree is appended to the target project's stored doc and blocked-by links that reached outside it are dropped on both sides (dependencies never span projects). The move is undoable via a toast that reverses both sides, with a "View" shortcut that switches to the destination.
