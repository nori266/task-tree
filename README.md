...
- **Leaf-fall (litter).** A done leaf is bright the day it's finished, faded the next, and on the day after that it lets go: it detaches, sways down out of the view, and comes to rest on the Forest floor as litter. Ages count in local calendar days, so "yesterday" means yesterday however late you worked. Falling happens only on app open and on the first return on a new day — never under your cursor mid-session. A parent emptied by a fall is skipped by that same sweep, so a finished branch erodes from the tips inward over successive mornings instead of vanishing in a clump; one emptied parent that never had a status of its own inherits `done`, so the Backlog can't claim it as abandoned work. Litter banks at the foot of its branch's tree once that branch graduates, and otherwise drifts in a band along the bottom of the Forest tab with a running count. An undo toast is the only way back, since litter has no per-item UI.
